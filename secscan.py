...
class Router(threading.Thread):
    ...
    def __init__(self, queue, user, passw):
        if not PARAMIKO_IMPORTED:
            ...
            print 'https://www.lag.net/paramiko/'
            sys.exit(1)
...
class Crawl:
    ...
    def crawler(self):
        ...
        for i in range(int(self.pages)):
            host = "https://uk.ask.com/web?q=%s&page=%s" % (str(self.qdork), self.page)
...
def admin():
    ...
    print 'Need to include https:// and ending /\n'
    site = raw_input('Site: ')
    queue  = Queue.Queue()
...
class OnlineCrack:
    ...
    def crack(self):
        ...
        hashm = raw_input('Enter MD5 Hash: ')
        conn = urllib2.Request('https://md5.hashcracking.com/search.php?md5=%s' % (hashm))
        conn.add_header('User-Agent', choice(USER_AGENT))
        opener = urllib2.build_opener()
        opener.open(conn)
        data = opener.open(conn).read()
